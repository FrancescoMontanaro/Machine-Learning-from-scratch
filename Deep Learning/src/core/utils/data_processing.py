...
def apply_rope_embeddings(x: Tensor, freqs_cis: Tensor) -> Tensor:
    """
    Applies rotary positional embeddings to the input Tensor.
    
    Args:
    - x (Tensor): Input tensor of shape (B, S, num_heads, head_dim).
    - freqs_cis (Tensor): Precomputed cos/sin values of shape (S, head_dim).
    ...
    *batch_dims, head_dim = x.shape
    ...
    x_reshape = x.reshape((*batch_dims, half_dim, 2))
    ...
    freqs_cos = freqs_cis[:, :half_dim].reshape((1, -1, 1, half_dim))
    freqs_sin = freqs_cis[:, half_dim:].reshape((1, -1, 1, half_dim))
    ...
    out = stack([out_real, out_imag], axis=-1)
