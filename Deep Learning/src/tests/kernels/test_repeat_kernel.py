...
class TestRepeatKernel(Test):
    ...
    def test_repeat_forward_values(self):
        """
        Test that repeat_forward matches torch.repeat_interleave on forward pass.
        """
        ...
        # PyTorch forward
        out_torch = torch.repeat_interleave(x_torch, self.repeats, dim=self.axis).detach().numpy()

        # Custom forward
        out_custom = x_custom.repeat(self.repeats, axis=self.axis).detach().to_numpy()
    ...
    def test_repeat_backward_values(self):
        """
        Test that repeat_backward matches torch.repeat_interleave backward gradients.
        """
        ...
        out_torch = torch.repeat_interleave(x_torch, self.repeats, dim=self.axis)
        out_torch.backward(torch.ones_like(out_torch))

        # Custom forward + backward
        out_custom = x_custom.repeat(self.repeats, axis=self.axis)
        out_custom.backward()
    ...
    def test_repeat_performance(self):
        ...
        for _ in range(n_iters):
            _ = torch.repeat_interleave(x, self.repeats, dim=self.axis)
        ...
        for _ in range(n_iters):
            _ = x.repeat(self.repeats, axis=self.axis)
        t_custom_fwd = time.time() - start

        # Print performance results
        print(f"torch.repeat_interleave forward: {t_torch_fwd:.6f}s, custom: {t_custom_fwd:.6f}s")
        ...
        self.assertLess(
            ...
            msg=(
                ...
                f"torch.repeat_interleave: {t_torch_fwd:.6f}s, custom: {t_custom_fwd:.6f}s"
            )
        )
        ...
        for _ in range(n_iters):
            out = torch.repeat_interleave(x, self.repeats, dim=self.axis)
        ...
        for _ in range(n_iters):
            out = x.repeat(self.repeats, axis=self.axis)
            out.backward()
        t_custom_bwd = time.time() - start

        # Print performance results
        print(f"torch.repeat_interleave backward: {t_torch_bwd:.6f}s, custom: {t_custom_bwd:.6f}s")
        ...
        self.assertLess(
            ...
            msg=(
                ...
                f"torch.repeat_interleave backward: {t_torch_bwd:.6f}s, custom: {t_custom_bwd:.6f}s"
            )
        )
