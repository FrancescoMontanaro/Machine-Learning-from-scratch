...
class TestMeanKernel(Test):
    ...
    def test_mean_forward_values(self):
        """
        Test that mean_forward matches torch.mean on forward pass.
        """
        ...
        # PyTorch forward
        out_torch = torch.mean(x_torch, dim=1).detach().numpy()

        # Custom forward
        out_custom = x_custom.mean(axis=1).detach().to_numpy()
    ...
    def test_mean_backward_values(self):
        """
        Test that mean_backward matches torch.mean backward gradients.
        """
        ...
        out_torch = torch.mean(x_torch, dim=1)
        out_torch.backward(torch.ones_like(out_torch))

        # Custom forward + backward
        out_custom = x_custom.mean(axis=1)
        out_custom.backward()
    ...
    def test_mean_performance(self):
        ...
        print(f"torch.mean forward: {t_torch_fwd:.6f}s, custom: {t_custom_fwd:.6f}s")
        ...
        self.assertLess(
            ...
            msg=(
                ...
                f"torch.mean: {t_torch_fwd:.6f}s, custom: {t_custom_fwd:.6f}s"
            )
        )
        ...
        # Print performance results
        print(f"torch.mean backward: {t_torch_bwd:.6f}s, custom: {t_custom_bwd:.6f}s")
        ...
        self.assertLess(
            ...
            msg=(
                ...
                f"torch.mean backward: {t_torch_bwd:.6f}s, custom: {t_custom_bwd:.6f}s"
            )
        )
