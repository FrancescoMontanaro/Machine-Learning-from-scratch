...
class TestSqrtKernel(Test):
    ...
    def test_sqrt_forward_values(self):
        """
        Test that sqrt_forward matches torch.sqrt on forward pass.
        """
        ...
        # PyTorch forward
        out_torch = torch.sqrt(x_torch).detach().numpy()

        # Custom forward
        out_custom = x_custom.sqrt().detach().to_numpy()
    ...
    def test_sqrt_backward_values(self):
        """
        Test that sqrt_backward matches torch.sqrt backward gradients.
        """
        ...
        out_torch = torch.sqrt(x_torch)
        out_torch.backward(torch.ones_like(out_torch))

        # Custom forward + backward
        out_custom = x_custom.sqrt()
        out_custom.backward()
    ...
    def test_sqrt_performance(self):
        ...
        print(f"torch.sqrt forward: {t_torch_fwd:.6f}s, custom: {t_custom_fwd:.6f}s")
        ...
        self.assertLess(
            ...
            msg=(
                ...
                f"torch.sqrt: {t_torch_fwd:.6f}s, custom: {t_custom_fwd:.6f}s"
            )
        )
        ...
        # Print performance results
        print(f"torch.sqrt backward: {t_torch_bwd:.6f}s, custom: {t_custom_bwd:.6f}s")
        ...
        self.assertLess(
            ...
            msg=(
                ...
                f"torch.sqrt backward: {t_torch_bwd:.6f}s, custom: {t_custom_bwd:.6f}s"
            )
        )
