...
class TestLogSoftmaxKernel(Test):
    ...
    def test_log_softmax_forward_values(self):
        """
        Test that log_softmax_forward matches torch.log_softmax on forward pass.
        """
        ...
        # PyTorch forward
        out_torch = F.log_softmax(x_torch, dim=-1).detach().numpy()

        # Custom forward
        out_custom = x_custom.log_softmax(axis=-1).detach().to_numpy()
    ...
    def test_log_softmax_backward_values(self):
        """
        Test that log_softmax_backward matches torch.log_softmax backward gradients.
        """
        ...
        out_torch = F.log_softmax(x_torch, dim=-1)
        out_torch.backward(torch.ones_like(out_torch))

        # Custom forward + backward
        out_custom = x_custom.log_softmax(axis=-1)
        out_custom.backward()
    ...
    def test_log_softmax_performance(self):
        ...
        for _ in range(n_iters):
            _ = x.log_softmax(axis=-1)
        t_custom_fwd = time.time() - start

        # Print performance results
        print(f"torch.log_softmax forward: {t_torch_fwd:.6f}s, custom: {t_custom_fwd:.6f}s")
        ...
        self.assertLess(
            ...
            msg=(
                ...
                f"torch.log_softmax: {t_torch_fwd:.6f}s, custom: {t_custom_fwd:.6f}s"
            )
        )
        ...
        # Print performance results
        print(f"torch.log_softmax backward: {t_torch_bwd:.6f}s, custom: {t_custom_bwd:.6f}s")
        ...
        self.assertLess(
            ...
            msg=(
                ...
                f"torch.log_softmax backward: {t_torch_bwd:.6f}s, custom: {t_custom_bwd:.6f}s"
            )
        )
