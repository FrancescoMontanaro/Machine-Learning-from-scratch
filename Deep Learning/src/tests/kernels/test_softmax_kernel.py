...
class TestSoftmaxKernel(Test):
    ...
    def test_softmax_forward_values(self):
        """
        Test that softmax_forward matches torch.softmax on forward pass.
        """
        ...
        # PyTorch forward
        out_torch = F.softmax(x_torch, dim=-1).detach().numpy()

        # Custom forward
        out_custom = x_custom.softmax(axis=-1).detach().to_numpy()
    ...
    def test_softmax_backward_values(self):
        """
        Test that softmax_backward matches torch.softmax backward gradients.
        """
        ...
        out_custom = x_custom.softmax(axis=-1)
        out_custom.backward()
    ...
    def test_softmax_performance(self):
        ...
        print(f"torch.softmax forward: {t_torch_fwd:.6f}s, custom: {t_custom_fwd:.6f}s")
        ...
        self.assertLess(
            ...
            msg=(
                ...
                f"torch.softmax: {t_torch_fwd:.6f}s, custom: {t_custom_fwd:.6f}s"
            )
        )
        ...
        # Print performance results
        print(f"torch.softmax backward: {t_torch_bwd:.6f}s, custom: {t_custom_bwd:.6f}s")
        ...
        self.assertLess(
            ...
            msg=(
                ...
                f"torch.softmax backward: {t_torch_bwd:.6f}s, custom: {t_custom_bwd:.6f}s"
            )
        )
