...
class TestClipKernel(Test):
    ...
    def test_clip_forward_values(self):
        """
        Test that clip_forward matches torch.clamp on forward pass.
        """
        ...
        # PyTorch forward
        out_torch = torch.clamp(x_torch, self.min_val, self.max_val).detach().numpy()

        # Custom forward
        out_custom = x_custom.clip(self.min_val, self.max_val).detach().to_numpy()
    ...
    def test_clip_backward_values(self):
        """
        Test that clip_backward matches torch.clamp backward gradients.
        """
        ...
        out_torch = torch.clamp(x_torch, self.min_val, self.max_val)
        out_torch.backward(torch.ones_like(out_torch))

        # Custom forward + backward
        out_custom = x_custom.clip(self.min_val, self.max_val)
        out_custom.backward()
    ...
    def test_clip_performance(self):
        ...
        for _ in range(n_iters):
            _ = torch.clamp(x, self.min_val, self.max_val)
        ...
        for _ in range(n_iters):
            _ = x.clip(self.min_val, self.max_val)
        t_custom_fwd = time.time() - start

        # Print performance results
        print(f"torch.clamp forward: {t_torch_fwd:.6f}s, custom: {t_custom_fwd:.6f}s")
        ...
        self.assertLess(
            ...
            msg=(
                ...
                f"torch.clamp: {t_torch_fwd:.6f}s, custom: {t_custom_fwd:.6f}s"
            )
        )
        ...
        for _ in range(n_iters):
            out = torch.clamp(x, self.min_val, self.max_val)
        ...
        for _ in range(n_iters):
            out = x.clip(self.min_val, self.max_val)
            out.backward()
        t_custom_bwd = time.time() - start

        # Print performance results
        print(f"torch.clamp backward: {t_torch_bwd:.6f}s, custom: {t_custom_bwd:.6f}s")
        ...
        self.assertLess(
            ...
            msg=(
                ...
                f"torch.clamp backward: {t_torch_bwd:.6f}s, custom: {t_custom_bwd:.6f}s"
            )
        )
