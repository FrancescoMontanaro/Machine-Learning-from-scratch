...
class TestFlattenKernel(Test):
    ...
    def setUp(self):
        ...
        self.x_raw = np.random.randn(200, 10, 5).astype(np.float32)


    def test_flatten_forward_values(self):
        """
        Test that flatten_forward matches torch.flatten on forward pass.
        """
        ...
        # PyTorch forward
        out_torch = torch.flatten(x_torch, start_dim=1, end_dim=2).detach().numpy()

        # Custom forward
        out_custom = x_custom.flatten(start_dim=1, end_dim=2).detach().to_numpy()
    ...
    def test_flatten_backward_values(self):
        """
        Test that flatten_backward matches torch.flatten backward gradients.
        """
        ...
        out_torch = torch.flatten(x_torch, start_dim=1, end_dim=2)
        out_torch.backward(torch.ones_like(out_torch))

        # Custom forward + backward
        out_custom = x_custom.flatten(start_dim=1, end_dim=2)
        out_custom.backward()
    ...
    def test_flatten_performance(self):
        ...
        for _ in range(n_iters):
            _ = torch.flatten(x, start_dim=1, end_dim=2)
        ...
        for _ in range(n_iters):
            _ = x.flatten(start_dim=1, end_dim=2)
        t_custom_fwd = time.time() - start

        # Print performance results
        print(f"torch.flatten forward: {t_torch_fwd:.6f}s, custom: {t_custom_fwd:.6f}s")
        ...
        self.assertLess(
            ...
            msg=(
                ...
                f"torch.flatten: {t_torch_fwd:.6f}s, custom: {t_custom_fwd:.6f}s"
            )
        )
        ...
        for _ in range(n_iters):
            out = x.flatten(start_dim=1, end_dim=2)
            out.backward()
        t_custom_bwd = time.time() - start

        # Print performance results
        print(f"torch.flatten backward: {t_torch_bwd:.6f}s, custom: {t_custom_bwd:.6f}s")
        ...
        self.assertLess(
            ...
            msg=(
                ...
                f"torch.flatten backward: {t_torch_bwd:.6f}s, custom: {t_custom_bwd:.6f}s"
            )
        )
