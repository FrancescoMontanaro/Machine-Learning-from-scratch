...
class TestTanhKernel(Test):
    ...
    def test_tanh_forward_values(self):
        """
        Test that tanh_forward matches torch.tanh on forward pass.
        """
        ...
        # PyTorch forward
        out_torch = torch.tanh(x_torch).detach().numpy()

        # Custom forward
        out_custom = x_custom.tanh().detach().to_numpy()
    ...
    def test_tanh_backward_values(self):
        """
        Test that tanh_backward matches torch.tanh backward gradients.
        """
    ...
    def test_tanh_performance(self):
        """
        ...
        print(f"torch.tanh forward: {t_torch_fwd:.6f}s, custom: {t_custom_fwd:.6f}s")
        ...
        self.assertLess(
            ...
            msg=(
                ...
                f"torch.tanh: {t_torch_fwd:.6f}s, custom: {t_custom_fwd:.6f}s"
            )
        )
        ...
        # Print performance results
        print(f"torch.tanh backward: {t_torch_bwd:.6f}s, custom: {t_custom_bwd:.6f}s")
        ...
        self.assertLess(
            ...
            msg=(
                ...
                f"torch.tanh backward: {t_torch_bwd:.6f}s, custom: {t_custom_bwd:.6f}s"
            )
        )
