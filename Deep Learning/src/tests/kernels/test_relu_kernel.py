...
class TestReluKernel(Test):
    ...
    def test_relu_forward_values(self):
        """
        Test that relu_forward matches torch.relu on forward pass.
        """
        ...
        # PyTorch forward
        out_torch = torch.relu(x_torch).detach().numpy()

        # Custom forward
        out_custom = x_custom.relu().detach().to_numpy()
    ...
    def test_relu_backward_values(self):
        """
        Test that relu_backward matches torch.relu backward gradients.
        """
    ...
    def test_relu_performance(self):
        """
        ...
        print(f"torch.relu forward: {t_torch_fwd:.6f}s, custom: {t_custom_fwd:.6f}s")
        ...
        self.assertLess(
            ...
            msg=(
                ...
                f"torch.relu: {t_torch_fwd:.6f}s, custom: {t_custom_fwd:.6f}s"
            )
        )
        ...
        # Print performance results
        print(f"torch.relu backward: {t_torch_bwd:.6f}s, custom: {t_custom_bwd:.6f}s")
        ...
        self.assertLess(
            ...
            msg=(
                ...
                f"torch.relu backward: {t_torch_bwd:.6f}s, custom: {t_custom_bwd:.6f}s"
            )
        )
