...
class TestMaskedFillKernel(Test):
    ...
    def setUp(self):
        ...
        self.x_raw = np.random.randn(200, 100).astype(np.float32)
        self.mask_raw = (np.random.rand(200, 100) > 0.5)
        self.fill_value = -1.0


    def test_masked_fill_forward_values(self):
        """
        Test that masked_fill_forward matches torch.masked_fill on forward pass.
        """
        ...
        # PyTorch forward
        out_torch = x_torch.masked_fill(torch.tensor(self.mask_raw), self.fill_value).detach().numpy()

        # Custom forward
        out_custom = x_custom.masked_fill(self.mask_raw, self.fill_value).detach().to_numpy()
    ...
    def test_masked_fill_backward_values(self):
        """
        Test that masked_fill_backward matches torch.masked_fill backward gradients.
        """
        ...
        mask_torch = torch.tensor(self.mask_raw)

        # PyTorch forward + backward
        out_torch = x_torch.masked_fill(mask_torch, self.fill_value)
        out_torch.backward(torch.ones_like(out_torch))

        # Custom forward + backward
        out_custom = x_custom.masked_fill(self.mask_raw, self.fill_value)
        out_custom.backward()
    ...
    def test_masked_fill_performance(self):
        """
        ...
        mask = torch.tensor(self.mask_raw)
        for _ in range(n_iters):
            _ = x.masked_fill(mask, self.fill_value)
        ...
        for _ in range(n_iters):
            _ = x.masked_fill(self.mask_raw, self.fill_value)
        t_custom_fwd = time.time() - start

        # Print performance results
        print(f"torch.masked_fill forward: {t_torch_fwd:.6f}s, custom: {t_custom_fwd:.6f}s")
        ...
        self.assertLess(
            ...
            msg=(
                ...
                f"torch.masked_fill: {t_torch_fwd:.6f}s, custom: {t_custom_fwd:.6f}s"
            )
        )
        ...
        mask = torch.tensor(self.mask_raw)
        for _ in range(n_iters):
            out = x.masked_fill(mask, self.fill_value)
        ...
        for _ in range(n_iters):
            out = x.masked_fill(self.mask_raw, self.fill_value)
            out.backward()
        t_custom_bwd = time.time() - start

        # Print performance results
        print(f"torch.masked_fill backward: {t_torch_bwd:.6f}s, custom: {t_custom_bwd:.6f}s")
        ...
        self.assertLess(
            ...
            msg=(
                ...
                f"torch.masked_fill backward: {t_torch_bwd:.6f}s, custom: {t_custom_bwd:.6f}s"
            )
        )
