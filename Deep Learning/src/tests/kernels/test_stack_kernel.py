...
class TestStackKernel(Test):
    ...
    def setUp(self):
        ...
        # Tensors share all dimensions except the one along which we stack
        self.x_raw = np.random.rand(100, 30, 40).astype(np.float32)
        self.y_raw = np.random.rand(100, 30, 40).astype(np.float32)

        # Stack along this axis
        self.stack_axis = 0


    def test_stack_forward_values(self):
        """
        Test that stack_forward matches torch.cat on forward pass.
        """
        ...
        # PyTorch forward
        z_torch = torch.stack([x_torch, y_torch], dim=self.stack_axis)
        out_torch = z_torch.detach().numpy()

        # Custom forward
        out_custom = Tensor.stack([x_tensor, y_tensor], axis=self.stack_axis)
    ...
    def test_stack_backward_values(self):
        """
        Test that stack_forward matches torch.cat backward gradients.
        """
        ...
        z_custom = Tensor.stack([x_custom, y_custom], axis=self.stack_axis)
        z_custom.backward()
    ...
    def test_stack_performance(self):
        ...
        for _ in range(n_iters):
            _ = torch.stack([x, y], dim=self.stack_axis)
        ...
        for _ in range(n_iters):
            _ = Tensor.stack([x, y], axis=self.stack_axis)
        t_custom_fwd = time.time() - start

        # Print performance results
        print(f"torch.cat forward: {t_torch_fwd:.6f}s, stack_forward: {t_custom_fwd:.6f}s")
        ...
        self.assertLess(
            ...
            msg=(
                ...
                f"torch.cat: {t_torch_fwd:.6f}s, stack_forward: {t_custom_fwd:.6f}s"
            )
        )
        ...
        for _ in range(n_iters):
            z = torch.stack([x, y], dim=self.stack_axis)
        ...
        for _ in range(n_iters):
            out = Tensor.stack([x, y], axis=self.stack_axis)
            out.backward()
        t_custom_bwd = time.time() - start

        # Print performance results
        print(f"torch.cat backward: {t_torch_bwd:.6f}s, stack_backward: {t_custom_bwd:.6f}s")
        ...
        self.assertLess(
            ...
            msg = (
                ...
                f"torch.cat backward: {t_torch_bwd:.6f}s, stack_backward: {t_custom_bwd:.6f}s"
            )
        )
