...
class TestExpKernel(Test):
    ...
    def test_exp_forward_values(self):
        """
        Test that exp_forward matches torch.exp on forward pass.
        """
        ...
        # PyTorch forward
        out_torch = torch.exp(x_torch).detach().numpy()

        # Custom forward
        out_custom = x_custom.exp().detach().to_numpy()
    ...
    def test_exp_backward_values(self):
        """
        Test that exp_backward matches torch.exp backward gradients.
        """
        ...
        out_torch = torch.exp(x_torch)
        out_torch.backward(torch.ones_like(out_torch))

        # Custom forward + backward
        out_custom = x_custom.exp()
        out_custom.backward()
    ...
    def test_exp_performance(self):
        ...
        print(f"torch.exp forward: {t_torch_fwd:.6f}s, custom: {t_custom_fwd:.6f}s")
        ...
        self.assertLess(
            ...
            msg=(
                ...
                f"torch.exp: {t_torch_fwd:.6f}s, custom: {t_custom_fwd:.6f}s"
            )
        )
        ...
        # Print performance results
        print(f"torch.exp backward: {t_torch_bwd:.6f}s, custom: {t_custom_bwd:.6f}s")
        ...
        self.assertLess(
            ...
            msg=(
                ...
                f"torch.exp backward: {t_torch_bwd:.6f}s, custom: {t_custom_bwd:.6f}s"
            )
        )
