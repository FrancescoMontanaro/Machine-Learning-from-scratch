...
class TestPowKernel(Test):
    ...
    def test_pow_forward_values(self):
        """
        Test that pow_forward matches torch.pow on forward pass.
        """
        ...
        # PyTorch forward
        out_torch = torch.pow(x_torch, self.power).detach().numpy()

        # Custom forward
        out_custom = (x_custom ** self.power).detach().to_numpy()
    ...
    def test_pow_backward_values(self):
        """
        Test that pow_backward matches torch.pow backward gradients.
        """
        ...
        out_torch = torch.pow(x_torch, self.power)
        out_torch.backward(torch.ones_like(out_torch))

        # Custom forward + backward
        out_custom = x_custom ** self.power
        out_custom.backward()
    ...
    def test_pow_performance(self):
        ...
        print(f"torch.pow forward: {t_torch_fwd:.6f}s, custom: {t_custom_fwd:.6f}s")
        ...
        self.assertLess(
            ...
            msg=(
                ...
                f"torch.pow: {t_torch_fwd:.6f}s, custom: {t_custom_fwd:.6f}s"
            )
        )
        ...
        # Print performance results
        print(f"torch.pow backward: {t_torch_bwd:.6f}s, custom: {t_custom_bwd:.6f}s")
        ...
        self.assertLess(
            ...
            msg=(
                ...
                f"torch.pow backward: {t_torch_bwd:.6f}s, custom: {t_custom_bwd:.6f}s"
            )
        )
