...
class TestSumKernel(Test):
    ...
    def test_sum_forward_values(self):
        """
        Test that sum_forward matches torch.sum on forward pass.
        """
        ...
        # PyTorch forward
        out_torch = torch.sum(x_torch, dim=1).detach().numpy()

        # Custom forward
        out_custom = x_custom.sum(axis=1).detach().to_numpy()
    ...
    def test_sum_backward_values(self):
        """
        Test that sum_backward matches torch.sum backward gradients.
        """
        ...
        out_torch = torch.sum(x_torch, dim=1)
        out_torch.backward(torch.ones_like(out_torch))

        # Custom forward + backward
        out_custom = x_custom.sum(axis=1)
        out_custom.backward()
    ...
    def test_sum_performance(self):
        ...
        print(f"torch.sum forward: {t_torch_fwd:.6f}s, custom: {t_custom_fwd:.6f}s")
        ...
        self.assertLess(
            ...
            msg=(
                ...
                f"torch.sum: {t_torch_fwd:.6f}s, custom: {t_custom_fwd:.6f}s"
            )
        )
        ...
        # Print performance results
        print(f"torch.sum backward: {t_torch_bwd:.6f}s, custom: {t_custom_bwd:.6f}s")
        ...
        self.assertLess(
            ...
            msg=(
                ...
                f"torch.sum backward: {t_torch_bwd:.6f}s, custom: {t_custom_bwd:.6f}s"
            )
        )
