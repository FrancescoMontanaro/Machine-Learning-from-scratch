...
class TestSqueezeKernel(Test):
    ...
    def setUp(self):
        ...
        self.x_raw = np.random.randn(50, 1, 20).astype(np.float32)


    def test_squeeze_forward_values(self):
        """
        Test that squeeze_forward matches torch.squeeze on forward pass.
        """
        ...
        # PyTorch forward
        out_torch = torch.squeeze(x_torch, dim=1).detach().numpy()

        # Custom forward
        out_custom = x_custom.squeeze(axis=1).detach().to_numpy()
    ...
    def test_squeeze_backward_values(self):
        """
        Test that squeeze_backward matches torch.squeeze backward gradients.
        """
        ...
        out_torch = torch.squeeze(x_torch, dim=1)
        out_torch.backward(torch.ones_like(out_torch))

        # Custom forward + backward
        out_custom = x_custom.squeeze(axis=1)
        out_custom.backward()
    ...
    def test_squeeze_performance(self):
        """
        ...
        print(f"torch.squeeze forward: {t_torch_fwd:.6f}s, custom: {t_custom_fwd:.6f}s")
        ...
        self.assertLess(
            ...
            msg=(
                ...
                f"torch.squeeze: {t_torch_fwd:.6f}s, custom: {t_custom_fwd:.6f}s"
            )
        )
        ...
        for _ in range(n_iters):
            out = x.squeeze(axis=1)
            out.backward()
        t_custom_bwd = time.time() - start

        # Print performance results
        print(f"torch.squeeze backward: {t_torch_bwd:.6f}s, custom: {t_custom_bwd:.6f}s")
        ...
        self.assertLess(
            ...
            msg=(
                ...
                f"torch.squeeze backward: {t_torch_bwd:.6f}s, custom: {t_custom_bwd:.6f}s"
            )
        )
